...
* (text): synchronous features call ``/sdapi/v1/cloud/*``
  directly from the frontend - no V2 job-queue path needed.
* (image): ``execute_cloud_image`` will dispatch to
  ``modules.cloud.image.generate_image``.
* (video): ``execute_cloud_video`` will dispatch to
  ``modules.cloud.video.generate_video``.
* (audio): ``execute_cloud_tts`` and ``execute_cloud_stt`` will
...
def execute_cloud_image(params: dict, job_id: str) -> dict:
    raise NotImplementedError("cloud_image executor stubbed during Phase 1 cloud cutover; modules.cloud.image not yet wired")
...
def execute_cloud_tts(params: dict, job_id: str) -> dict:
    raise NotImplementedError("cloud_tts executor stubbed during Phase 1 cloud cutover; modules.cloud.audio.tts not yet wired")


def execute_cloud_stt(params: dict, job_id: str) -> dict:
    raise NotImplementedError("cloud_stt executor stubbed during Phase 1 cloud cutover; modules.cloud.audio.stt not yet wired")


def execute_cloud_video(params: dict, job_id: str) -> dict:
    raise NotImplementedError("cloud_video executor stubbed during Phase 1 cloud cutover; modules.cloud.video not yet wired")
