...
export function QuickSettingsTab() {
  ...
  const groups = useMemo(() => {
    ...
    return Array.from(sectionMap.entries()).map(([title, settings]) => ({
      title,
      settings,
    }));
  ...
  const allPickerItems = useMemo(() => {
    ...
    for (const [key, info] of Object.entries(meta)) {
      ...
      items.push({
        ...
        section: info.section_title || "Other",
      });
    }
    return items;
  }, [optionsInfo, options]);
  ...
  const handleChange = useCallback(
    (key: string, value: unknown, isSlider: boolean) => {
      if (isSlider) {
          ...
          <div className="space-y-1.5">
            {group.settings.map((setting) => (
              <div key={setting.key} className="flex items-center gap-2">
                <span
                  className="text-2xs text-muted-foreground w-[6.875rem] shrink-0 truncate"
                  title={setting.label}
                >
                  {setting.label}
                </span>
