...
export function MaskParams() {
  ...
  const handleInvertToggle = useCallback(
    (checked: boolean) => setInpaintingMaskInvert(checked),
    [setInpaintingMaskInvert],
  );
  const handleOverlayToggle = useCallback(
    (checked: boolean) => setMaskApplyOverlay(checked),
    [setMaskApplyOverlay],
  );
  ...
  return (
    <SectionLeader title="Mask" collapsible>
      <ParamSlider
        ...
        max={64}
        step={1}
        tooltip={getParamHelp("mask blur")}
      />
      ...
      <ParamSlider
        ...
        max={256}
        step={4}
        disabled={!inpaintFullRes}
        tooltip={getParamHelp("mask padding")}
      />
      ...
      <ParamSlider
        label="Mask strength"
        value={inpaintingMaskWeight}
        onChange={setInpaintingMaskWeight}
        min={0}
        max={1}
        step={0.01}
      />
