...
const PARAM_GROUPS: { label: string; keys: string[] }[] = [
  ...
  { label: "Guidance", keys: ["CFG true", "CFG adaptive", "Image CFG scale"] },
  {
    ...
    keys: [
      "Sampler sigma",
      "Sampler spacing",
      "Sampler beta schedule",
      "Sampler type",
      "Sampler shift",
      "Sampler dynamic shift",
      "Sampler low order",
      "Sampler dynamic",
      "Sampler rescale",
      "Sampler order",
  ...
  {
    label: "Model",
    keys: ["Model", "Model hash", "VAE", "VAE type", "Pipeline", "TE", "UNet"],
  },
];
...
function PromptSection({
  ...
  return (
    <div className="space-y-1.5">
      ...
      <div className="bg-muted/30 rounded p-2 text-2xs whitespace-pre-wrap break-words max-h-32 overflow-y-auto">
        {text}
      </div>
    </div>
  );
...
export function PngInfoDialog({ open, onOpenChange }: PngInfoDialogProps) {
  ...
  const grouped = PARAM_GROUPS.map((g) => ({
    label: g.label,
    entries: resolveGroupEntries(paramStrings, g.keys),
  }));
  ...
  const otherEntries: [string, string][] = [
    ...
    ...Object.entries(items)
      .filter(([key, val]) => !paramStrings[key] && val)
    ...
    <Dialog open={open} onOpenChange={handleClose}>
      ...
      >
        <DialogDescription className="sr-only">
          View and restore generation parameters from PNG image metadata
        </DialogDescription>
                ...
                <div className="p-4 space-y-3">
                  {pngInfo.isPending ? (
                    ...
                    <p className="text-xs text-muted-foreground">
                      No generation info found in this image.
                    </p>
                  ) : (
                    <>
                      {/* Prompt */}
                      ...
                      {otherEntries.length > 0 && (
                        <ParamGroup label="Other" entries={otherEntries} />
                      )}
                    </>
                  )}
                </div>
