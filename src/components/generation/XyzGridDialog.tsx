        ...
        <div className="pl-8 space-y-1">
          ...
          {validation?.response && !validation.response.ok && (
            <div className="text-2xs text-destructive">
              {validation.response.errors.map((e, i) => (
                <div key={i}>{e}</div>
              ))}
            </div>
          )}
          {validation?.response?.ok && (
            <div className="text-2xs text-muted-foreground">
              Resolved: {validation.response.resolved.map(String).join(", ")} (
              {validation.response.count} values)
            </div>
          )}
        </div>
      )}
...
export function XyzGridDialog({
  ...
  const [xValidation, setXValidation] = useState<AxisValidation>({
    isLoading: false,
  });
  const [yValidation, setYValidation] = useState<AxisValidation>({
    isLoading: false,
  });
  const [zValidation, setZValidation] = useState<AxisValidation>({
    isLoading: false,
  });
  ...
  const findOpt = useCallback(
    (label: string) => allOptions.find((o) => o.label === label),
    [allOptions],
  );
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Sweep parameters across X, Y, and Z axes in a single grid job.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-3 py-2">
          <AxisSection
            label="X"
            axisType={xType}
            onAxisTypeChange={setXType}
            values={xValues}
            onValuesChange={setXValues}
            groups={groups}
            allOptions={allOptions}
            validation={xValidation}
            onValidate={() => handleValidate(xType, xValues, setXValidation)}
          />

          <AxisSection
            label="Y"
            axisType={yType}
            onAxisTypeChange={setYType}
            values={yValues}
            onValuesChange={setYValues}
            groups={groups}
            allOptions={allOptions}
            validation={yValidation}
            onValidate={() => handleValidate(yType, yValues, setYValidation)}
          />
          ...
          {showZ ? (
            <>
              <AxisSection
                label="Z"
                axisType={zType}
                onAxisTypeChange={setZType}
                values={zValues}
                onValuesChange={setZValues}
                groups={groups}
                allOptions={allOptions}
            ...
            {showOptions && (
              <div className="mt-2 space-y-2 pl-1">
                ...
                {showZ && (
                  <label className="flex items-center gap-2 text-2xs">
                    <Checkbox
                      checked={includeSubgrids}
                      onCheckedChange={(v) => setIncludeSubgrids(!!v)}
                    />
                    Include sub-grids
                  </label>
                )}
