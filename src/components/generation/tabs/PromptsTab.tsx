...
export function PromptsTab() {
  ...
  const cloudSizePresets = useMemo(() => {
    ...
    // Phase 2.5: size_constraint is authoritative when present.
    ...
    if (sc?.kind === "bucket" && sc.options.length > 0) {
      ...
      // per SPEC §11.10.11 A2. Requires plumbing an activeBucketSymbol state
  ...
  // Echoed-size surfacing (SPEC §11.10.10 F3): when the server returns dims
