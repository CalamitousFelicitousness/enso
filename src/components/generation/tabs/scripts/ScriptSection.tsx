        ...
        <div className="flex items-center gap-4">
          ...
          {layout.headerRow!.map((idx) => {
            ...
            return (
              <label
                key={idx}
                className={`flex items-center gap-1.5 text-2xs text-muted-foreground cursor-pointer${disabled ? " opacity-50 pointer-events-none" : ""}`}
              >
                <Switch
                  size="sm"
                  checked={Boolean(getArgValue(idx) ?? arg.value)}
                  onCheckedChange={(checked) => setArgValue(idx, checked)}
                  disabled={disabled}
                />
