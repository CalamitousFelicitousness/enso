...
export function MergeSubTab() {
  ...
  const vaeOptions = [
    { value: "", label: "(None)" },
    ...(vaes ?? []).map((v) => ({ value: v.name, label: v.name })),
  ];
    ...
    <div className="space-y-3">
      ...
      {isTriple && (
        <div>
          ...
          <Combobox
            value={tertiary}
            onValueChange={setTertiary}
            options={modelOptions}
            placeholder="Select model C..."
            className="h-6 text-2xs"
          />
        </div>
      )}
        ...
        <div className="space-y-2 pl-3">
          ...
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              ...
              <Label htmlFor="merge-overwrite" className="text-2xs">
                Overwrite
              </Label>
            </div>
            <div className="flex items-center gap-2">
              <Checkbox
                id="merge-savemeta"
                checked={saveMeta}
                onCheckedChange={(v) => setSaveMeta(!!v)}
              />

              <Label htmlFor="merge-savemeta" className="text-2xs">
                Save metadata
              </Label>
            </div>
            <div className="flex items-center gap-2">
              <Checkbox
                id="merge-wclip"
                checked={weightsClip}
                onCheckedChange={(v) => setWeightsClip(!!v)}
              />

              <Label htmlFor="merge-wclip" className="text-2xs">
                Weights clip
              </Label>
            </div>
            <div className="flex items-center gap-2">
            ...
            <div className="flex items-center gap-2">
              <Checkbox
                id="merge-unload"
                checked={unload}
                onCheckedChange={(v) => setUnload(!!v)}
              />

              <Label htmlFor="merge-unload" className="text-2xs">
            ...
            <div className="flex items-center gap-2">
              <Checkbox
                id="merge-rebasin"
                checked={reBasin}
                onCheckedChange={(v) => setReBasin(!!v)}
              />

              <Label htmlFor="merge-rebasin" className="text-2xs">
                ReBasin
              </Label>
            </div>
            {reBasin && (
              <div className="pl-5">
                ...
                <Slider
                  value={[reBasinIter]}
                  onValueChange={([v]) => setReBasinIter(v)}
                  min={1}
                  max={20}
                  step={1}
                  className="mt-1"
      ...
      <Button
        ...
        variant="default"
        onClick={handleMerge}
        disabled={merge.isPending || !customName || !primary || !secondary}
        className="w-full"
      >
