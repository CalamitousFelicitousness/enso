  ...
  return (
    <div className="border border-border/50 rounded-lg overflow-hidden">
      ...
      >
        ...
        {hasSome && (
          <span
            title={`${downloadedCount}/${totalFiles} file${totalFiles > 1 ? "s" : ""} downloaded`}
          >
        ...
        <div className="px-4 pb-4 space-y-4">
          ...
          <ImageStrip
            images={version.images
              .filter((img) => !img.url.toLowerCase().endsWith(".mp4"))
              .slice(0, 10)}
            className="!h-24 [&_img]:h-24"
            onImageClick={
              onImageClick
                ? (i) => {
                    const filtered = version.images
                      .filter((img) => !img.url.toLowerCase().endsWith(".mp4"))
                      .slice(0, 10);
                  ...
                  <div className="flex flex-wrap gap-1.5">
                    {version.trainedWords.map((w, i) => (
                      <Badge
                        ...
                        onClick={() => {
                          appendToGenerationPrompt(w);
                          toast.success("Added to prompt", { description: w });
                        }}
                      >
  ...
  return (
    ...
    >
      <DialogContent
        ...
        className="sm:max-w-5xl max-h-[85vh] flex flex-col p-0 gap-0 overflow-hidden"
            ...
            <div className="shrink-0 flex items-center gap-3 px-5 py-3 border-b border-border/50 bg-background">
              ...
              <div className="flex items-center gap-1 shrink-0 ml-2">
                <Button
                  ...
                  onClick={() =>
                    isBookmarked
                      ? removeBookmark.mutate(model.name)
                      : addBookmark.mutate(model.name)
                  }
                ...
                >
                  <Ban
                    className={`h-4 w-4 ${isBanned ? "fill-orange-500 text-orange-500" : "text-muted-foreground"}`}
                  />
                </Button>
                <Button
                  size="icon"
              ...
              <DialogDescription className="sr-only">
                Model details and download options
              </DialogDescription>
            </div>
