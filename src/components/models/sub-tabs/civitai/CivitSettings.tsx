      ...
      {open && (
        <div className="px-3 pb-3 space-y-4">
          ...
          <div className="flex items-center justify-between gap-2">
            <Label htmlFor="subfolder-enabled" className="text-xs">
              Save downloads into subfolders
            </Label>
            <Switch
          ...
          {settings.save_subfolder_enabled && (
            <SubfolderSection
              value={settings.save_subfolder}
              onSave={(v) => save.mutate({ save_subfolder: v })}
              isPending={save.isPending}
            />
          )}
          <div className="flex items-center justify-between gap-2">
            <Label htmlFor="hash-mismatch" className="text-xs">
              Discard downloads with hash mismatch
            </Label>
            <Switch
              id="hash-mismatch"
              size="sm"
              checked={settings.discard_hash_mismatch}
...
function TokenSection({ configured }: { configured: boolean }) {
    ...
    return (
      <div className="flex items-center gap-2">
        ...
        <Button
          size="sm"
          variant="ghost"
          className="h-6 text-3xs ml-auto"
          onClick={() => {
            setEditing(true);
            setSaved(false);
          }}
  ...
  return (
    <div className="space-y-1">
      <Label htmlFor="civit-token" className="text-xs">
        CivitAI API token
      </Label>
...
function SubfolderSection({
  ...
  return (
    <div className="space-y-1.5">
      <Label htmlFor="subfolder-template" className="text-xs">
        Save subfolder template
      </Label>
      <div className="flex gap-2">
        <Input
          ref={inputRef}
          id="subfolder-template"
          placeholder="e.g. {{BASEMODEL}}/{{CREATOR}}"
          value={draft}
