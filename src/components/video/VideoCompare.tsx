...
export function VideoCompare({ left, right }: VideoCompareProps) {
  ...
  // incumbent sets the frame length.
  const stepFps = left.fps ?? right.fps ?? DEFAULT_FPS;
  const hasMotion = !leftStill || !rightStill;
  ...
  const stepFrame = useCallback(
    (delta: number) => {
      bothVideos().forEach((v) => {
        v.pause();
        v.currentTime = Math.max(0, Math.min(v.duration, v.currentTime + delta / stepFps));
      });
    },
    [bothVideos, stepFps],
  );
    ...
    >
      ...
      <div className="flex-1 flex min-h-0">
        ...
        <div className="flex-1 relative">
          ...
          <div className="absolute top-2 left-2 px-2 py-0.5 rounded bg-black/60 text-white text-3xs">
            {compareLabel("B", right)}
          </div>
        </div>
      </div>

      {/* Shared control bar. Two stills have nothing to play. */}
      {hasMotion && (
