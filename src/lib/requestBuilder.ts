...
export async function buildControlRequest(): Promise<BuildResult> {
  ...
  if (isImg2Img) {
    ...
    // Export mask from the primary frame's mask state. will
    // fully migrate mask painting per-frame; for now sources
    ...
    if (primaryMaskData) {
      ...
      // legacy signature reads getMaskLayers globally - will
